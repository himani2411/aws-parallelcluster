...
def assert_imex_nodes_config_is_correct(rce: RemoteCommandExecutor, queue_name: str, compute_resource_name: str, expected_ips: list):
    logging.info(f"Checking IMEX nodes config contains the expected nodes: {expected_ips}")
    imex_nodes_config_file = f"/opt/parallelcluster/shared/nvidia-imex/nodes_config_{queue_name}_{compute_resource_name}.cfg"
