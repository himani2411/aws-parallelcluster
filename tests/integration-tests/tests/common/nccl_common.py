...
def install_and_run_nccl_benchmarks(remote_command_executor, mpi_module, scheduler_commands, instance):
    ...
    # try:
    jobs_ids_list = []
    gpu_per_node_list = [4,4,2,2] #get_instance_info(instance)["GpuInfo"]["Gpus"][0]["Count"]
    ...
    #     )
    #     gpu_per_node_list = get_instance_info(instance, "us-east-1")["GpuInfo"]["Gpus"][0]["Count"]

    for gpu_per_node in gpu_per_node_list:
        ...
        jobs_ids_list.append(scheduler_commands.assert_job_submitted(result.stdout))
    ...
    with soft_assertions():
        for job_id in jobs_ids_list:
            ...
            max_bandwidth = remote_command_executor.run_remote_command(
                "cat /shared/nccl_tests_{0}.out | grep -E '1073741824\\s+268435456' | awk '{print $12}'".format(job_id)
            ).stdout

            out_of_place_max_bandwidth = remote_command_executor.run_remote_command(
                "cat /shared/nccl_tests_{0}.out | grep -E '1073741824\\s+268435456' | awk '{print $8}'".format(job_id)
            ).stdout
